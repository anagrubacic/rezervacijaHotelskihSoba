 ...
 * @author user
 */
public class IzmenaManagera extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!name.equals("") && !lastname.equals("") &&
                    !newemail.equals("") && !password.equals("") &&
                    !username.equals("")) {
                try {
                    int hotel = Integer.parseInt(request.getParameter("hotel"));
                    String stmt = "UPDATE manager SET firstname=?,lastname=?,email=?,username=?,password=?,hotel=? where manager_id=?";
...
//                response.sendRedirect("AdministratorIndex.jsp");
                    String path = "izmeni_men.jsp?manager_id=" + manager_id;
                    request.getRequestDispatcher(path).forward(request, response);
                }
            } else {
                request.setAttribute("msg", "Sva polja su obavezna!");
//                response.sendRedirect("AdministratorIndex.jsp");
                String path = "IzmeniMenadzera.jsp?manager_id=" + manager_id;
                request.getRequestDispatcher(path).forward(request, response);
            }
        }
    }
