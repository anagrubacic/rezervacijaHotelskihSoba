 ...
 * @author user
 */
public class IzmenaKlijenta extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!name.equals("") && !lastname.equals("") && !email.equals("") && !password.equals("")) {
                try {
                    String stmt = "Update client set firstname=?,lastname=?,email=?,password=?,numberOfPoints=? where client_id=?";
                    ...
                    pst.executeUpdate();
                    response.sendRedirect("prikaz_klijenata.jsp");
                } catch (Exception ex) {
                    request.setAttribute("msg", ex.getCause());
                    request.getRequestDispatcher("izmeni_klijenta.jsp?client_id=" + id).forward(request, response);
                }
            } else {
                request.setAttribute("msg", "Sva polja su obavvezna!");
                request.getRequestDispatcher("izmeni_klijenta.jsp?client_id=" + id).forward(request, response);
            }
        }
    }
